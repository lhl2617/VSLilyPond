...
suite(`MIDIIn Test Suite`, () => {
    ...
    suite(`getNoteChar suite`, () => {
        const getNoteCharTestsException: {
        ...
        }[] = [
            {
                ...
                accidentals: `sharps`,
                noteNum: -1,
                error: new Error(`NoteNumber should be a integer within [0,11]; got -1`),
            },
            {
                name: "Exception number above 11",
                accidentals: `sharps`,
                noteNum: 12,
                error: new Error(`NoteNumber should be a integer within [0,11]; got 12`),
            },
            {
                name: "Number is float",
                accidentals: `sharps`,
                noteNum: 4.23,
                error: new Error(`NoteNumber should be a integer within [0,11]; got 4.23`),
            }
        ];
    ...
    suite(`getAbsoluteOctavePostfix suite`, () => {
        const getAbsoluteOctavePostfixTestsException: {
        ...
        }[] = [
            {
                name: "Below 0",
                octaveNum: -1,
                error: new Error(`OctaveNumber should be an integer within [0,9]; got -1`),
            },
            {
                name: "Above 10",
                octaveNum: 10,
                error: new Error(`OctaveNumber should be an integer within [0,9]; got 10`),
            },
            {
                name: "Float",
                octaveNum: 3.141,
                error: new Error(`OctaveNumber should be an integer within [0,9]; got 3.141`),
            },
        ];
    ...
    suite(`midiNumberToNoteName suite`, () => {
        const midiNumberToNoteNameTestsException: {
        ...
        }[] = [
            {
                ...
                accidentals: `sharps`,
                relativeMode: false,
                error: new Error(`MIDI Note should be an integer within [12, 127], got 11`)
            },
            {
                name: "Above 127",
                note: 128,
                accidentals: `sharps`,
                relativeMode: false,
                error: new Error(`MIDI Note should be an integer within [12, 127], got 128`)
            },
            {
                name: "Float",
                note: 69.42,
                accidentals: `sharps`,
                relativeMode: false,
                error: new Error(`MIDI Note should be an integer within [12, 127], got 69.42`)
            },
        ];
    ...
    suite(`processNote suite`, () => {
        ...
        for (const t of tests) {
            const { name, MIDINotes, MIDIInputConfig, want, wantActiveNotes, wantChordNotes } = t;
            test(name, () => {
